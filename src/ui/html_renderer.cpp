...
std::string generate_beavertask_planner_html(const TranslationCatalog& translations,
    ...
    const std::string hero_description = translations.translate("BeaverTask hero description", language);
    const std::string back_to_menu = translations.translate("Back to menu", language);
    const std::string language_label = translations.translate("Language selection", language);
    const std::string switch_to_french = translations.translate("Switch to French", language);
    const std::string switch_to_english = translations.translate("Switch to English", language);
    const std::string create_title = translations.translate("BeaverTask create title", language);
    const std::string create_description = translations.translate("BeaverTask create description", language);
    const std::string field_title_label = translations.translate("BeaverTask field title label", language);
    const std::string field_title_placeholder = translations.translate("BeaverTask field title placeholder", language);
    const std::string field_due_label = translations.translate("BeaverTask field due label", language);
    const std::string field_notes_label = translations.translate("BeaverTask field notes label", language);
    const std::string notes_placeholder = translations.translate("BeaverTask notes placeholder", language);
    const std::string add_button = translations.translate("BeaverTask add button", language);
    const std::string board_title = translations.translate("BeaverTask board title", language);
    const std::string empty_state = translations.translate("BeaverTask empty state", language);
    const std::string progress_summary = translations.translate("BeaverTask progress summary", language);
    const std::string mark_done = translations.translate("BeaverTask mark done", language);
    const std::string mark_active = translations.translate("BeaverTask mark active", language);
    const std::string delete_label = translations.translate("BeaverTask delete", language);
    const std::string no_due = translations.translate("BeaverTask focus no due", language);
    const std::string untitled = translations.translate("BeaverTask untitled", language);

    const std::string locale_code = (language == Language::French) ? "fr-CA" : "en-CA";

    std::vector<std::string> strings_entries;
    strings_entries.emplace_back(std::string("\"emptyState\": \"") + json_escape(empty_state) + "\"");
    strings_entries.emplace_back(std::string("\"progressSummary\": \"") + json_escape(progress_summary) + "\"");
    strings_entries.emplace_back(std::string("\"markDone\": \"") + json_escape(mark_done) + "\"");
    strings_entries.emplace_back(std::string("\"markActive\": \"") + json_escape(mark_active) + "\"");
    strings_entries.emplace_back(std::string("\"delete\": \"") + json_escape(delete_label) + "\"");
    strings_entries.emplace_back(std::string("\"noDue\": \"") + json_escape(no_due) + "\"");
    strings_entries.emplace_back(std::string("\"untitled\": \"") + json_escape(untitled) + "\"");

    std::ostringstream strings_payload;
    strings_payload << "{\n";
    for (std::size_t i = 0; i < strings_entries.size(); ++i) {
        strings_payload << "        " << strings_entries[i];
        if (i + 1 < strings_entries.size()) {
            strings_payload << ",";
        }
        strings_payload << "\n";
    }
    ...
    append("</head>");
    append("<body>");
    append("  <div class=\"bt-shell\" data-locale=\"" + html_escape(locale_code) + "\">");
    append("    <header class=\"bt-header\">");
    append("      <a class=\"bt-back\" href=\"" + menu_href + "\" aria-label=\"" + html_escape(back_to_menu) + "\">");
    append("        <span aria-hidden=\"true\">&#8592;</span>");
    append("      </a>");
    append("      <div class=\"bt-heading\">");
    append("        <h1 class=\"bt-title\">" + html_escape(beavertask_label) + "</h1>");
    append("        <p class=\"bt-subtitle\">" + html_escape(hero_description) + "</p>");
    append("      </div>");
    append("      <nav class=\"bt-language\" aria-label=\"" + html_escape(language_label) + "\">");
    append("        <a class=\"bt-language-link" + std::string((language == Language::French) ? " is-active" : "") + "\" href=\"" + beavertask_french_href + "\" aria-label=\"" + html_escape(switch_to_french) + "\"" + std::string((language == Language::French) ? " aria-current=\"page\"" : "") + ">FR</a>");
    append("        <a class=\"bt-language-link" + std::string((language == Language::English) ? " is-active" : "") + "\" href=\"" + beavertask_english_href + "\" aria-label=\"" + html_escape(switch_to_english) + "\"" + std::string((language == Language::English) ? " aria-current=\"page\"" : "") + ">EN</a>");
    append("      </nav>");
    append("    </header>");
    append("    <main class=\"bt-main\">");
    append("      <section class=\"bt-panel bt-panel-form\">");
    append("        <h2 class=\"bt-panel-title\">" + html_escape(create_title) + "</h2>");
    append("        <p class=\"bt-panel-description\">" + html_escape(create_description) + "</p>");
    append("        <form id=\"bt-form\" class=\"bt-form\" autocomplete=\"off\">");
    append("          <label class=\"bt-field\" for=\"bt-title\">");
    append("            <span class=\"bt-field-label\">" + html_escape(field_title_label) + "</span>");
    append("            <input id=\"bt-title\" name=\"title\" type=\"text\" placeholder=\"" + html_escape(field_title_placeholder) + "\" required maxlength=\"120\" class=\"bt-input\" />");
    append("          </label>");
    append("          <label class=\"bt-field\" for=\"bt-date\">");
    append("            <span class=\"bt-field-label\">" + html_escape(field_due_label) + "</span>");
    append("            <input id=\"bt-date\" name=\"date\" type=\"date\" class=\"bt-input\" />");
    append("          </label>");
    append("          <label class=\"bt-field\" for=\"bt-notes\">");
    append("            <span class=\"bt-field-label\">" + html_escape(field_notes_label) + "</span>");
    append("            <textarea id=\"bt-notes\" name=\"notes\" rows=\"4\" placeholder=\"" + html_escape(notes_placeholder) + "\" class=\"bt-textarea\"></textarea>");
    append("          </label>");
    append("          <button type=\"submit\" class=\"bt-primary\">" + html_escape(add_button) + "</button>");
    append("        </form>");
    append("      </section>");
    append("      <section class=\"bt-panel bt-panel-board\">");
    append("        <header class=\"bt-board-header\">");
    append("          <h2 class=\"bt-panel-title\">" + html_escape(board_title) + "</h2>");
    append("          <p id=\"bt-progress\" class=\"bt-progress\" aria-live=\"polite\"></p>");
    append("        </header>");
    append("        <div id=\"bt-task-list\" class=\"bt-task-list\" role=\"list\"></div>");
    append("        <p id=\"bt-empty\" class=\"bt-empty\" aria-live=\"polite\">" + html_escape(empty_state) + "</p>");
    append("      </section>");
    append("    </main>");
    append("  </div>");
    append("  <script>");
    append("    (function() {");
    append("      const root = document.querySelector('.bt-shell');");
    append("      if (!root) { return; }");
    append("      const strings = " + strings_payload.str() + ";");
    append("      const storageKey = \"beavertask.v2.tasks\";");
    append("      const form = document.getElementById('bt-form');");
    append("      const titleInput = document.getElementById('bt-title');");
    append("      const dateInput = document.getElementById('bt-date');");
    append("      const notesInput = document.getElementById('bt-notes');");
    append("      const taskList = document.getElementById('bt-task-list');");
    append("      const emptyStateEl = document.getElementById('bt-empty');");
    append("      const progressEl = document.getElementById('bt-progress');");
    append("      const locale = root.dataset.locale || 'en-CA';");
    append("      try {");
    append("        const dateFormatter = new Intl.DateTimeFormat(locale, { dateStyle: 'medium' });");
    append("        const state = { tasks: [] };");
    append("        const escapeHtml = (value) => {");
    append("          if (typeof value !== 'string') { return ''; }");
    append("          return value.replace(/[&<>\"']/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '\"': '&quot;', '\'': '&#39;' }[char] || char));");
    append("        };");
    append("        if (!form || !titleInput || !dateInput || !notesInput || !taskList || !progressEl) {");
    append("          if (form) { form.addEventListener('submit', (event) => event.preventDefault()); }");
    append("          console.warn('[BeaverTask] Missing planner nodes.');");
    append("          return;");
    append("        }");
    append("        const loadTasks = () => {");
    append("        try {");
    append("          const raw = window.localStorage.getItem(storageKey);");
    append("          if (!raw) { return []; }");
    append("          const parsed = JSON.parse(raw);");
    append("          if (!Array.isArray(parsed)) { return []; }");
    append("          return parsed.filter((item) => item && typeof item === 'object').map((item) => ({");
    ...
    append("            notes: typeof item.notes === 'string' ? item.notes : '',");
    append("            completed: Boolean(item.completed),");
    append("            createdAt: typeof item.createdAt === 'string' ? item.createdAt : new Date().toISOString()");
    append("          }));");
    append("        } catch (error) {");
    append("          console.warn('[BeaverTask] Failed to load tasks.', error);");
    append("          return [];");
    append("        }");
    append("      };");
    ...
    append("        }");
    append("      };");
    append("      const formatDue = (value) => {");
    append("        if (!value) { return strings.noDue || ''; }");
    append("        const parts = value.split('-').map((part) => Number(part));");
    append("        if (parts.length !== 3 || parts.some((part) => !Number.isFinite(part))) { return strings.noDue || ''; }");
    append("        const date = new Date(parts[0], parts[1] - 1, parts[2]);");
    append("        if (Number.isNaN(date.getTime())) { return strings.noDue || ''; }");
    append("        return dateFormatter.format(date);");
    append("      };");
    append("      const updateProgress = () => {");
    append("        const total = state.tasks.length;");
    append("        const completed = state.tasks.filter((task) => task.completed).length;");
    append("        if (progressEl) {");
    append("          progressEl.textContent = (strings.progressSummary || '').replace('%COMPLETED%', String(completed)).replace('%TOTAL%', String(total));");
    append("        }");
    append("      };");
    append("      const render = () => {");
    append("        updateProgress();");
    append("        if (!taskList) { return; }");
    append("        if (!state.tasks.length) {");
    append("          taskList.innerHTML = '';");
    append("          if (emptyStateEl) { emptyStateEl.hidden = false; emptyStateEl.textContent = strings.emptyState || ''; }");
    append("          return;");
    append("        }");
    append("        if (emptyStateEl) { emptyStateEl.hidden = true; }");
    append("        const html = state.tasks.map((task) => {");
    append("          const title = escapeHtml(task.title || strings.untitled || '');");
    append("          const due = escapeHtml(formatDue(task.dueDate));");
    append("          const notes = task.notes ? `<p class=\"bt-task-notes\">${escapeHtml(task.notes).replace(/\n/g, '<br />')}</p>` : '';");
    append("          const toggleLabel = escapeHtml(task.completed ? strings.markActive || '' : strings.markDone || '');");
    append("          return `<article class=\"bt-task${task.completed ? ' is-complete' : ''}\" data-id=\"${escapeHtml(task.id)}\" role=\"listitem\"><div class=\"bt-task-main\"><h3 class=\"bt-task-title\">${title}</h3><p class=\"bt-task-meta\">${due}</p>${notes}</div><div class=\"bt-task-actions\"><button type=\"button\" class=\"bt-task-toggle\" data-action=\"toggle\">${toggleLabel}</button><button type=\"button\" class=\"bt-task-delete\" data-action=\"delete\">${escapeHtml(strings.delete || '')}</button></div></article>`;");
    append("        }).join('');");
    append("        taskList.innerHTML = html;");
    append("      };");
    append("      const persistAndRender = () => { saveTasks(); render(); };");
    append("      form.addEventListener('submit', (event) => {");
    append("        event.preventDefault();");
    append("        const title = (titleInput.value || '').trim();");
    append("        const dueDate = (dateInput.value || '').trim();");
    append("        const notes = notesInput.value || '';");
    append("        if (!title) { titleInput.focus(); return; }");
    append("        state.tasks.unshift({");
    append("          id: `task_${Date.now().toString(36)}${Math.random().toString(36).slice(2, 8)}`,");
    append("          title,");
    append("          dueDate,");
    append("          notes,");
    append("          completed: false,");
    append("          createdAt: new Date().toISOString()");
    append("        });");
    append("        form.reset();");
    append("        titleInput.focus();");
    append("        persistAndRender();");
    append("      });");
    append("      taskList.addEventListener('click', (event) => {");
    append("        const button = event.target.closest('[data-action]');");
    append("        if (!button) { return; }");
    append("        const idElement = button.closest('.bt-task');");
    append("        if (!idElement) { return; }");
    append("        const id = idElement.getAttribute('data-id');");
    append("        if (!id) { return; }");
    append("        if (button.getAttribute('data-action') === 'toggle') {");
    append("          const task = state.tasks.find((item) => item.id === id);");
    append("          if (task) { task.completed = !task.completed; persistAndRender(); }");
    append("        } else if (button.getAttribute('data-action') === 'delete') {");
    append("          const index = state.tasks.findIndex((item) => item.id === id);");
    append("          if (index >= 0) { state.tasks.splice(index, 1); persistAndRender(); }");
    append("        }");
    append("      });");
    append("      state.tasks = loadTasks();");
    append("      render();");
    append("      } catch (error) {");
    append("        console.error('[BeaverTask] Failed to initialize planner.', error);");
    append("        if (form) {");
    append("          form.addEventListener('submit', (event) => event.preventDefault());");
    append("        }");
    append("      }");
    append("    })();");
    append("  </script>");
