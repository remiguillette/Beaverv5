...
std::string generate_beavertask_planner_html(const TranslationCatalog& translations,
    ...
    append("      const isFiniteNumber = typeof Number === 'object' && typeof Number.isFinite === 'function' ? Number.isFinite : (value) => isFinite(value);");
    append("      const isNumberNaN = typeof Number === 'object' && typeof Number.isNaN === 'function' ? Number.isNaN : (value) => typeof value === 'number' && isNaN(value);");
    append("      const parseInteger = typeof Number === 'object' && typeof Number.parseInt === 'function' ? Number.parseInt : (value, radix) => parseInt(value, radix || 10);");
    append("      const state = {");
    append("        tasks: [],");
    ...
    append("      };");
    append("      const padNumber = (value, width = 2) => {");
    append("        let stringValue;");
    append("        if (typeof value === 'number') {");
    append("          const finite = isFiniteNumber(value) ? value : 0;");
    append("          const safe = Math.floor(Math.abs(finite));");
    append("          stringValue = safe.toString();");
    append("        } else {");
    append("          stringValue = String(value || '');");
    append("        }");
    append("        if (stringValue.length >= width) {");
    append("          return stringValue;");
    append("        }");
    append("        while (stringValue.length < width) {");
    append("          stringValue = `0${stringValue}`;");
    append("        }");
    append("        return stringValue;");
    append("      };");
    append("      const getDateKey = (date) => {");
    append("        const year = date.getFullYear();");
    append("        const month = padNumber(date.getMonth() + 1);");
    append("        const day = padNumber(date.getDate());");
    ...
    append("          return null;");
    append("        }");
    append("        const parts = value.split('-').map((part) => parseInteger(part, 10));");
    append("        if (parts.length !== 3 || parts.some((part) => isNumberNaN(part))) {");
    append("          return null;");
    append("        }");
    ...
    append("            reminderMinutes: isFiniteNumber(Number(item.reminderMinutes)) ? Number(item.reminderMinutes) : 0,");
    ...
    append("        if (!isFiniteNumber(value) || value <= 0) {");
    append("          return '';");
    append("        }");
    ...
    append("        const reminderValue = taskReminderInput ? parseInteger(taskReminderInput.value, 10) : 0;");
    ...
    append("          reminderMinutes: isFiniteNumber(reminderValue) && reminderValue > 0 ? reminderValue : 0,");
